...
<a href="<?php echo $base_url?>/purchase" class="btn btn-secondary"> Purchase</a>
</div>
...
<div class="invoice-container">
	...
	<header class="invoice-header">
		...
		<div class="invoice-number">
			...
			<h3>Invoice #: 100<?php echo  Purchase::get_last_id()+1;?></h3>
			<h3><?php //echo  purchase::get_last_id()?></h3>
			<p>Date: <span id="purchase_date"><?php echo date("Y-m-d") ?></span></p>
		</div>
	</header>
	...
	<section class="invoice-address">
		<div class="address-column">
			...
			<p><strong >Name:<?php echo  Supplier::html_select("supplier_id") ?></strong></p>
		...
		<div class="address-column">
			...
			<p><strong >Name:<?php echo  Warehouse::html_select("warehouse_id") ?></strong></p>
	...
	//	alert();
		...
		$("#supplier_id").on("change", function() {
			...
			$.ajax({
				...
				success: function(res) {
					...
					let supplier_data = JSON.parse(res)
					
					$("#email").text(supplier_data.supplier.email);
					$("#phone").text(supplier_data.supplier.phone);
					$("#get_address").text(supplier_data.supplier.address);
					$("#address_cus").text(supplier_data.supplier.address);
				},
				error: function(res) {}
			})
		...
		$(".process").on("click", function() {
			...
			let supplier_id = $("#supplier_id").val();
			 let purchase_date = $("#purchase_date").text();
			let delivery_date = "";
			let shipping_address = $("#get_address").text();
			let purchase_total = $("#total_amount").text();
			let remark = "";
			let total_amount =$("#total_amount").text();
			let total_order =$("#total_order").text();
			let paid_amount = $("#paid_amount").text();
			// let total_discount = $("#discount_amount").val() ?? 0;
			let discount = $("#discount_amount").val() ?? 0;
			let vat = $("#tax_amount").text();
			let status_id = 2;
			let product = cart.getCart();

			//console.log(supplier_id, purchase_date, shipping_address, purchase_total, paid_amount, discount, vat, product);

			$.ajax({
				url: "<?php echo $base_url ?>/api/purchase/process",
				type: "post",
				data: {
				...
				success: function(res) {
					...
					cart.clearCart()
			        printCart()
					window.location.replace("<?php echo $base_url?>/purchase");
				},
				error: function(res) {
				    console.log(res)
				}
