...
class Order extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}orders(customer_id,order_date,delivery_date,shipping_address,order_total,paid_amount,remark,status_id,discount,vat)values('$this->customer_id','$this->order_date','$this->delivery_date','$this->shipping_address','$this->order_total','$this->paid_amount','$this->remark','$this->status_id','$this->discount','$this->vat')");
		return $db->insert_id;
	}
	...
	public static function revenue($criteria=""){
		global $db,$tx;
		$result =$db->query("select sum(order_total) count from {$tx}orders $criteria");
		list($count)=$result->fetch_row();
			return $count;
	}
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Customer Id</th><th>Order Date</th><th>Delivery Date</th><th>Shipping Address</th><th>Order Total</th><th>Paid Amount</th><th>Action</th></tr>";

			// <th>Remark</th>
		}else{
			$html.="<tr><th>Id</th><th>Customer Id</th><th>Order Date</th><th>Delivery Date</th><th>Shipping Address</th><th>Order Total</th><th>Paid Amount</th><th>Status Id</th><th>Discount</th><th>Vat</th></tr>";
		...
		while($order=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= Event::button(["name"=>"show", "value"=>"Show", "class"=>"btn btn-info", "route"=>"order/show?id=$order->id"]);
			...
			$html.="<tr><td>$order->id</td><td>$order->customer_id</td><td>$order->order_date</td><td>$order->delivery_date</td><td>$order->shipping_address</td><td>$order->order_total</td><td>$order->paid_amount</td> $action_buttons</tr>";
		}
		// <td>$order->remark</td>
