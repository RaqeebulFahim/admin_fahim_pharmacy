...
class TransactionType extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}transaction_types(name,factor,created_at,updated_at)values('$this->name','$this->factor','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}transaction_types set name='$this->name',factor='$this->factor',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,factor,created_at,updated_at from {$tx}transaction_types");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,factor,created_at,updated_at from {$tx}transaction_types $criteria limit $top,$perpage");
		$data=[];
		while($transactiontype=$result->fetch_object()){
			$data[]=$transactiontype;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,factor,created_at,updated_at from {$tx}transaction_types where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'>".Html::link(["class"=>"btn btn-success","route"=>"transactiontype/create","text"=>"New TransactionType"])."</th></tr>";
		if($action){
		...
		while($transactiontype=$result->fetch_object()){
			...
			$html.="<tr><td>$transactiontype->id</td><td>$transactiontype->name</td><td>$transactiontype->factor</td><td>$transactiontype->created_at</td><td>$transactiontype->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		...
		$html.="<tr><th>Factor</th><td>$transactiontype->factor</td></tr>";
		$html.="<tr><th>Created At</th><td>$transactiontype->created_at</td></tr>";
		$html.="<tr><th>Updated At</th><td>$transactiontype->updated_at</td></tr>";
