...
class Warehouse extends Model implements JsonSerializable{
	...
	public function set($id,$name,$address,$location,$created_at,$updated_at){
		$this->id=$id;
		$this->name=$name;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}warehouses(name,address,location,created_at,updated_at)values('$this->name','$this->address','$this->location','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}warehouses set name='$this->name',address='$this->address',location='$this->location',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,address,location,created_at,updated_at from {$tx}warehouses");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,address,location,created_at,updated_at from {$tx}warehouses $criteria limit $top,$perpage");
		$data=[];
		while($warehouse=$result->fetch_object()){
			$data[]=$warehouse;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,address,location,created_at,updated_at from {$tx}warehouses where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Address</th><th>Location</th><th>Created At</th><th>Updated At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Address</th><th>Location</th><th>Created At</th><th>Updated At</th></tr>";
		}
		while($warehouse=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$warehouse->id</td><td>$warehouse->name</td><td>$warehouse->address</td><td>$warehouse->location</td><td>$warehouse->created_at</td><td>$warehouse->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
